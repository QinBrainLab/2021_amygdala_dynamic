...
%% conduct Time-lagged cross correlation (TLCC)
 
% compute TLCC using whole concatenated time sereis during each state
clear CTLL_inte_result CTLL_segre_result CTLL_inte_before CTll_inte_zero CTLL_inte_after CTLL_segre_before CTll_segre_zero CTLL_segre_after

lag=40;
    for i = 1:lag
        CTLL_inte_before(i)=corr(a1((1+lag-i+1):end),a2(1:(end-lag+i-1)),'Type','Pearson');
        CTll_inte_zero=corr(a1,a2,'Type','Pearson');
        CTLL_inte_after(i)=corr(a1((1+i):end),a2(1:(end-i)),'Type','Pearson');
        CTLL_segre_before(i)=corr(b1((1+lag-i+1):end),b2(1:(end-lag+i-1)),'Type','Pearson');
        CTll_segre_zero=corr(b1,b2,'Type','Pearson');
        CTLL_segre_after(i)=corr(b1((1+i):end),b2(1:(end-i)),'Type','Pearson');
    end
    CTLL_inte_result = [CTLL_inte_before,CTll_inte_zero,CTLL_inte_after];
    CTLL_segre_result = [CTLL_segre_before,CTll_segre_zero,CTLL_segre_after];
    
    plot(CTLL_inte_result)
    hold on
    plot(CTLL_segre_result)

%  compute windowed-TLCC
 clear CTLL_window_result_a   CTLL_window_result_b
 corr_win = 40;
 lag=40;
 window_num_a = floor(length(a1)/(corr_win+2*lag));
for j = 1:window_num_a
    a11 = a1(1+(corr_win+2*lag)*(j-1):((corr_win+2*lag)*j));
    a22 = a2(1+(corr_win+2*lag)*(j-1):((corr_win+2*lag)*j));
    for i = 1:lag
...
window_num_b = floor(length(b1)/(corr_win+2*lag));
for j = 1:window_num_b
    b11 = b1(1+(corr_win+2*lag)*(j-1):((corr_win+2*lag)*j));
    b22 = b2(1+(corr_win+2*lag)*(j-1):((corr_win+2*lag)*j));
    for i = 1:lag
...
% plot averaged WTLCC for each satte
x=[-40:40];
shadedErrorBar(x,mean(CTLL_window_result_a,1),std(CTLL_window_result_a)/sqrt(window_num_a),'lineProps','r');
hold on
shadedErrorBar(x,mean(CTLL_window_result_b,1),std(CTLL_window_result_b)/sqrt(window_num_b),'lineProps','b');
...
for i=1:41
    [h,p,ci,stats] = ttest(CTLL_a_z(:,i));
    t_stats(1,i) = stats.tstat;
    t_result(2,i) = p;
    clear g p ci stats
end